...
| M11c | **Done 2026-09-11.** `app/plex.py` (plex.tv PIN flow, account, resources, shared users; PMS library sections), `app/plex_access.py` (policy: first Plex sign-in on a fresh install = owner/admin; then owner, shared users of the chosen server, or anyone if allowed), `/login/plex` + callback with purposes signin/owner/link, `/api/auth/plex/pin` + `/api/auth/plex` for native clients (202 while unclaimed), Plex panel in Settings (connect/disconnect owner, server + library picker via `/api/plex/servers|sections`, allow-any toggle), Plex buttons on `/login` and `/setup`, link-from-profile; `tests/mock_plex.py` stands in for plex.tv and a PMS; migration `d4e5f6a7b8c9` | M |
| M11d | **Done 2026-09-11.** `app/tmdb.py` rails (trending, popular/upcoming movies, popular/on-the-air series), multi-search, movie/TV details (cast, trailer, genres, seasons, external ids, recommendations) behind a TTL cache; `app/routers/discover.py`: `/` Discover home (backdrop hero, **Recommended for you** from the library's recent titles, rails, library status merged onto every card), `/discover/search`, `/discover/movie|tv/{id}` detail pages with an availability card, admin **Add to library** (TV maps TMDB→TVmaze by TVDB id, IMDb id, then exact title), `/api/discover/*` JSON; `Series.tmdb_id` (migration `e5f6a7b8c9d0`); `tests/mock_tmdb.py` covers it all offline. Verified against real TMDB. | M |
| M11e | **Done 2026-09-11.** `app/plex_scan.py` walks the chosen libraries (`includeGuids=1`, paginated; TMDB/TVDB/IMDb ids from `Guid` tags or legacy agent guids; per-season episode counts for shows) into `plex_media`; scheduler job every `plex_scan_interval_minutes` (default 30, Settings → Plex), **Scan now** button, `POST/GET /api/plex/scan`; Discover cards say *available* / *partial* for Plex-held titles, the detail page has a Plex row and per-season "on Plex" badges, Recommended for you skips Plex titles. Migration `f6a7b8c9d0e1`. `tests/mock_plex.py` now serves a mock PMS library. | M |
| M11f | **Done 2026-09-11.** `media_requests` model + `app/requests_service.py` (availability = Den ∪ Plex; refuses what's already available or already requested; TV requests carry a season list, defaulting to every season not yet covered; auto-approve for admins and `auto_approve` users; Discord notify on new/approved/declined); `/requests` page (filters, approve, decline with a note, withdraw, per-user view) and `/api/requests`; Request button + season dialog on detail pages; approval creates the Movie or maps TMDB→TVmaze and creates the Series with `Episode.monitored` set only for the wanted seasons (existing series get the new seasons monitored); automation only grabs monitored episodes; pending-count badge on the sidebar for admins. 36-check e2e against mock Plex + real TMDB/TVmaze. | L |
| M11g | **Done 2026-09-12.** Request quotas (Settings → Requests defaults `request_movie_limit`/`request_series_limit`/`request_limit_days`, per-user overrides from the Users page "limits" dialog, admins exempt, declined requests don't count, 429 when exhausted, `/api/requests/quota` and `quota` on `/api/auth/me`, quota card on Profile and a remaining-count line on Requests); **available detection** (`requests_service.mark_available` at the end of every automation cycle and Plex scan stamps approved requests `available` + `available_at` and posts a Discord "now available" once; `?status=available` is a real filter); **sign-in toggle** in Settings → Accounts (`settings.auth_required` overrides the `AUTH_REQUIRED` env var via `auth.required()`, loaded at startup; only a signed-in admin may turn it on; `auth_required` on `/api/settings` and `/health`). Auto-approve and Discord request notifications landed in M11f; the profile page + API token in M11b. Migration `a7b8c9d0e1f2`. 61-check e2e. The env default stays `AUTH_REQUIRED=false` until the clients (U4/U5) have a login step; the toggle lets a household flip it earlier. | M |
