...
| U4 | **Done 2026-09-12** in `client/`: Login (Plex PIN + local → personal API token as `X-Api-Key`), glass sidebar shell with rail mode and the top-bar primary action, `holt/` QML component library mirroring the web names, Discover / Search / Detail (season picker) / Requests pages, Movies & TV poster grids, Episodes, Releases, Calendar agenda, Downloads on `/torrents`, Settings rebuilt on `/api/settings`; `JsonListModel` base removes the duplicated models and error blocks; tokens read from `design/exports/holt_tokens.py`. Verified offscreen: 13 pages compile with zero QML warnings and a full-app screenshot walk (`client/tests/run_all.sh`). Glass is a drawn ground layer, no live blur yet; the real-desktop pass is still to do. | L | U0, M11 |
