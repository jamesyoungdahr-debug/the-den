...
| U4 | KDE client (now in `client/` of this repo, merged 2026-09-12): login with the API token, Settings against the current `/api/settings`, then shell + component library + poster pages + Discover/Requests + Downloads on `/torrents`; visual pass on a real desktop. Tokens come straight from `design/exports/holt_tokens.py`. | L | U0, M11 |
