...
| U1 | **Done 2026-09-11.** `base.html` is the Glass shell (glass sidebar with Browse/Library/Admin groups + collapse-to-rail, glass top bar with search + primary-action slot, ground layer, bottom tab bar + More sheet under 900px); `den.css` is the v2 component layer (PosterCard, Rail, Hero, Badge, Chip, Row, Progress, Stat, Dialog, Toast, Skeleton, EmptyState; v1 class names kept); `den.js` for sidebar/dialog/toast. Home is a library-at-a-glance Discover (hero + rails), Movies/TV are poster grids with real TMDB/TVmaze art and hover actions, Indexers moved to `/ui/indexers` (M11a done with it). Static links are cache-busted by asset mtime. | L | U0 |
| U2 | **Done 2026-09-11.** Downloads (session stats with drawn rate history, Active/Seeding/Stopped groups, icon actions, add-torrent dialog, toasts), Settings (sectioned with a sticky side index, Save in the top bar, Accounts card), Calendar (real month grid with have/missing/upcoming, month navigation, agenda of missing + upcoming episodes and missing movies), Series detail (poster header with progress, season accordion), Candidates (release rows with quality chip, best match, sizes). Empty states and skeletons throughout; reduced-motion/transparency are token-level. | M | U1 |
| U3 | **Done 2026-09-11** alongside M11b-f: Login/Setup, Users, Discover, detail, Requests all built on the U1 components. | -- | U1 + M11 backend |
| U4 | **Done 2026-09-12** in the `the-den-client` repo (v0.4.2): Login (Plex PIN + local → personal API token as `X-Api-Key`), glass sidebar shell with rail mode and the top-bar primary action, `holt/` QML component library mirroring the web names, Discover / Search / Detail (season picker) / Requests pages, Movies & TV poster grids, Episodes, Releases, Calendar agenda, Downloads on `/torrents`, Settings rebuilt on `/api/settings`; `JsonListModel` base removes the duplicated models and error blocks; tokens are a committed copy of `design/exports/holt_tokens.py`. Verified offscreen: 13 pages compile with zero QML warnings and a full-app screenshot walk (`tests/run_all.sh` in that repo). Glass is a drawn ground layer, no live blur yet; the real-desktop pass is still to do. | L | U0, M11 |
| U5 | **Done 2026-09-12** in the `the-den-android` repo (v0.5.0): login gate (Plex PIN in the system browser + local account → personal API token as `X-Api-Key`), four-tab shell (Discover / Requests / Library / More; bottom `NavigationBar` on phones, `NavigationRail` from 600 dp) with the admin pages behind More, `ui/holt/` Compose component library mirroring the web and QML names, `HoltTokens.kt` generated from `design/exports`, Discover rails + hero + search, Detail with availability panel and season request sheet, Requests (approve / decline / withdraw, quota line), Library Movies/TV tabs on the merged `/api/library/*` feeds with Plex-only titles and proxied posters, Episodes, Releases, Calendar, Downloads (live), Indexers, Settings. Verified: `assembleDebug` clean and `LiveBackendTest` 5/5 against the dev backend with sign-in required. Emulator screenshot pass still to do. | L | U0, M11 |
