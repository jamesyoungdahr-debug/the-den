...
| E6 (done 2026-09-13, M30) | **Subtitles**: Bazarr-style OpenSubtitles fetch per language preference. Shipped: server-side fetch (OpenSubtitles REST v1) + a web Settings panel. Not shipped: KDE/Android settings UI -- unlike E4/E7/E8's KDE/Android omissions (which are deliberate, documented scope cuts matching that E-row's own surfaces column), this one is a real gap against the surfaces column ("settings on all surfaces") that was missed during a mid-session repo-recovery and should be finished if this is picked back up. | server, settings on all surfaces |
