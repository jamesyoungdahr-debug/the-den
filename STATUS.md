...
**Next: M50b, matching and collections (Plex replacement).** M50a (the file scan) is done; see its entry below. M50b matches the recorded files to TMDB/TVmaze, adds an unmatched queue, builds collections and points request availability at The Den's own library instead of the Plex scan. M51 (accounts without Plex) needs no decisions from Liam either. Plan and open questions: `docs/plex-replacement-plan.md`.
