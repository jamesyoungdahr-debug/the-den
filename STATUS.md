...
**Next: replacing Plex completely (planned 2026-09-15).** Liam's goal is to remove Plex Media Server and its apps and web player. Plex and The Den run side by side first; Plex is switched off at M60 and its code removed at M61. The order starts with G0 (an HTTPS serving benchmark), M50a and M50b (The Den's own library scanner) and M51 (accounts without Plex). Plan and open questions: `docs/plex-replacement-plan.md`.
