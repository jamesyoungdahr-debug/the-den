...
**Next: M51, accounts without Plex.** M50a (the file scan) and M50b (matching, the unmatched queue, collections) are done, and a first slice of transcoding now lets the web player play files a browser would otherwise refuse; see the entries below. M51 gives every Plex-linked user a Den sign-in, adds per-user library access, and replaces the Plex watchlist import with The Den's own watchlist. It needs none of Liam's pending decisions. Plan and open questions: `docs/plex-replacement-plan.md`.
