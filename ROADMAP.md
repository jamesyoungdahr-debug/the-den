...
- [x] M24 -- Per-title history (done 2026-09-13, E5 in docs/feature-research.md): a `history_events` table records every grab, upgrade, import, failure and removal against a movie/episode/series, shown as a History panel on the Detail page/screen on the web, KDE and Android
...
- [x] M29 -- Request comments and a 4K flag (done 2026-09-13, E8 in docs/feature-research.md): a user-visible comment thread on each request (requester + any admin), and an informational "request in 4K" flag, on the web and Android (no KDE for this one, per feature-research.md's surfaces column)
- [x] M30 -- Subtitles via OpenSubtitles (done 2026-09-13, E6 in docs/feature-research.md; documentation gap fixed 2026-09-13 -- this was actually implemented and pushed earlier, alongside E1/E2/E4, but missed a DONE entry at the time): fetches missing-language .srt files as siblings of each video, best-effort at the end of every automation cycle, server + web only (no KDE/Android settings UI built for this one despite the surfaces column calling for it -- a real gap, not a documented scope cut)
- [x] M31 -- Per-user ntfy topic for request outcomes (done 2026-09-13, E9 in docs/feature-research.md): a personal ntfy.sh topic on the profile page (web) and account page (Android) sends the signed-in user a push the moment one of their own requests is approved, declined or available, alongside whatever the admin-configured notification agents already fan out to; server and web + Android only (no KDE for this one, per feature-research.md's surfaces column)
- [x] M32 -- Backup and restore (done 2026-09-13, E10 in docs/feature-research.md): a Backup panel on web Settings downloads a consistent copy of the database, exports settings and the indexer list as JSON with secrets left out, and stages an uploaded database backup that replaces the live one at the next restart (the old one is kept as a .pre-restore copy); server and web only
- [x] M33 -- Sign-in always required, first-run setup (done 2026-09-13, first milestone of [docs/connect-secure-plan.md](docs/connect-secure-plan.md)): no anonymous access; a two-step web setup (admin account, then server name and library folders) must be finished before the server answers any app; server (0.8.0 track), web, KDE 0.5.0, Android 0.7.0
